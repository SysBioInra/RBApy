...
class Macrocomponents(object):
    ...
    def _read_curated_data(self, known_species):
        ...
        try:
            ...
            print('Found file with macrocomponent data. This file will be used to setup macrocomponent production fluxes...')
            # skip header
            try:
                next(input_stream)
            ...
            # write header
            output_stream.write('\t'.join(['TARGET_METABOLITE', 'TARGET_FLUX'])
                                + '\n')
            output_stream.close()
