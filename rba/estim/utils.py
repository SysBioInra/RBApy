...
def create_fba_problem(sbml_file, biomass_reaction):
    ...
    lp_problem.variables.add(names=reaction_ids)
    lp_problem.variables.set_lower_bounds(zip(reaction_ids, LB))
    lp_problem.variables.set_upper_bounds(zip(reaction_ids, UB))

    lp_problem.objective.set_sense(lp_problem.objective.sense.maximize)
    lp_problem.objective.set_linear(zip(reaction_ids, obj_fun_vec))

    lp_problem.linear_constraints.add(names=species_ids)
    lp_problem.linear_constraints.set_linear_components(zip(species_ids, cplex_rows))
    lp_problem.linear_constraints.set_rhs(zip(species_ids, RHS))
    lp_problem.linear_constraints.set_senses(zip(species_ids, ['E'] * len(species)))
