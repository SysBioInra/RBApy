...
class Processes(object):
    def __init__(self, data, species, reactions, functions):
        ...
        # extract machinery
        machinery = [p.machinery.machinery_composition \
        ...
        self.target_values = TargetValues(data, species, functions)
        # extract target reactions
        target_reactions = [r for p in processes for r in p.targets.reaction_fluxes]
        self.reaction_names = [t.reaction for t in target_reactions]
        self.reaction_index = [reactions.index(n) for n in self.reaction_names]
        ...
        self.lb_index = [reactions.index(t.reaction) for t in target_reactions]
        ...
        self.ub_index = [reactions.index(t.reaction) for t in target_reactions]
        self.ub = TargetVector(target_reactions, functions)

class TargetValues(object):
    def __init__(self, data, species, functions):
        targets = [p.targets for p in data.processes.processes]
